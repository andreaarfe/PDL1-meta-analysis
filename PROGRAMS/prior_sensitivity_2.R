...
shape  <- matrix(log(2)/12,nrow=length(cuts)-1,ncol=length(breaks))
rate   <- matrix(1,nrow=length(cuts)-1,ncol=length(breaks))

# Fit the model
# Uses diffuse priors distributions for the PD-L1 prevalence
...
save(fit, file="./DATA/prior_sensitivity_2.Rdata")
